...
async function main() {
  ...
  const email = `stage5-smoke-${Date.now()}@somni.test`;
  const password = `SomniSmoke!${Math.floor(Math.random() * 100000)}`;
  ...
  try {
    ...
    const { data: createdUser, error: createUserError } = await supabaseAdmin.auth.admin.createUser({
      ...
      password,
      email_confirm: true,
      user_metadata: { full_name: "Stage5 Smoke" },
    });
    ...
    console.log("Step 7: Verifying Dashboard load...");
    const dashResponse = await fetch(`http://127.0.0.1:${port}/dashboard`, {
      headers: { Cookie: cookieHeader }
    });
    const dashBody = await dashResponse.text();
    if (!dashResponse.ok || !dashBody.includes("<html")) {
       throw new Error(`Dashboard failed to load (${dashResponse.status})`);
    }

    console.log("Step 8: Verifying Sleep Log insertion...");
    const { error: sleepLogError } = await supabaseAdmin.from("sleep_logs").insert({
      baby_id: babyData.id,
    ...
    if (sleepLogError) throw new Error(`Sleep log insertion failed: ${sleepLogError.message}`);

    console.log("Step 9: Testing Support route...");
    const supportResponse = await fetch(`http://127.0.0.1:${port}/api/support`, {
      method: "POST",
      headers: { "Content-Type": "application/json", Cookie: cookieHeader },
      body: JSON.stringify({
        category: "bug",
        message: "This is a test bug report from the smoke suite.",
        originPage: "/dashboard"
      })
    });
    if (!supportResponse.ok) {
       throw new Error(`Support route failed (${supportResponse.status})`);
    }

    console.log("Stage 5 Smoke Verification Passed:");
    console.log("- Signup/Signin succeeded");
    console.log("- Authenticated chat streaming succeeded");
    console.log("- Dashboard loaded successfully");
    console.log("- Sleep log inserted successfully");
